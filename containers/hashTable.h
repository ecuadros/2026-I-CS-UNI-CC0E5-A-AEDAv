...
// ---------------------------------------------------------------------------
// HashBucket  (AVL tree used as a bucket)
// ---------------------------------------------------------------------------
template<typename Key, typename Value>
...
public:
    ...
    Node* findNode(const Key& key) const {
        auto* cur = static_cast<Node*>(this->m_pRoot);
        while (cur) {
            if (key == cur->m_data)  return cur;
            cur = static_cast<Node*>(cur->m_pChild[key < cur->m_data ? 0 : 1]);
        }
        return nullptr;
    }
    ...
    void insertKV(const Key& key, const Value& value, Ref ref = Ref{}) {
        ...
        this->insert(key, ref);          // inserta el nodo en el AVL
        found = findNode(key);
...
private:
    ...
    Bucket              *m_buckets;
    size_t               m_capacity;
    size_t               m_size;      // número de claves únicas
...
public:
    ...
    struct Iterator {
        HashTable *m_table;
        size_t     m_bucket;   // índice del bucket actual
        Node      *m_node;     // puntero al nodo actual (nunca nullptr salvo end())

        KVPair<Key,Value> operator*() const {
            return {m_node->m_data, m_node->m_value};
        }

        // Avanza al siguiente nodo en inorder dentro del mismo bucket,
        // o salta al primer nodo del siguiente bucket no vacío.
        Iterator& operator++() {
            // Pedimos el inorder del bucket actual y buscamos el sucesor.
            // Como BinaryTree::inorder() devuelve una vista que podemos
            // iterar, simplificamos buscando en todos los nodos del bucket
            // el siguiente en orden al actual.  Sigue siendo O(k) por bucket
            // pero ya no hay recorrido O(n²) por llamada.
            Node* next = nullptr;
            m_table->m_buckets[m_bucket].inorder().forEachNode([&](Node& n){
                if (n.m_data > m_node->m_data) {
                    if (!next || n.m_data < next->m_data)
                        next = &n;
                }
            });

            if (next) { m_node = next; return *this; }

            // No hay sucesor en este bucket; ir al siguiente bucket no vacío.
            ++m_bucket;
            while (m_bucket < m_table->m_capacity) {
                Node* first = nullptr;
                m_table->m_buckets[m_bucket].inorder().forEachNode([&](Node& n){
                    if (!first || n.m_data < first->m_data)
                        first = &n;
                });
                if (first) { m_node = first; return *this; }
                ++m_bucket;
            }
            m_node = nullptr;   // => end()
            return *this;
    ...
    Iterator begin() {
        for (size_t i = 0; i < m_capacity; ++i) {
            ...
            if (first) return Iterator{this, i, first};
        }
        return end();
    }
    Iterator end() { return Iterator{this, m_capacity, nullptr}; }

    // -----------------------------------------------------------------------
    // Constructores / destructor
    // -----------------------------------------------------------------------
    explicit HashTable(size_t capacity = kDefaultCapacity)
    ...
    void insert(const Key& key, const Value& value, Ref ref = Ref{}) {
        ...
        if (!m_buckets[idx].findNode(key)) ++m_size;   // solo cuenta claves nuevas
    ...
    Value& operator[](const Key& key) {
        ...
        // Clave nueva: insertar con valor por defecto y contar UNA sola vez.
    ...
    friend istream& operator>>(istream& is, HashTable& t) {
        ...
        Key key; Value val; T2 sep;
        while (is >> ch && ch != '}') {
            if (ch != ',') is.putback(ch);
            if (is >> key >> sep >> val && sep == ':')
                t.insert(key, val);   // insert() ya toma su propio lock
        }
        return is;
    }
};
